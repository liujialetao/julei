...
def convert_examples_to_features(examples, tokenizer,
    ...
    import json
    with open('/hy-tmp/julei/bert_torch_clf/label_map.json', 'w') as f_obj:
        json.dump(label_map, f_obj)
    features = []
...
class THUNewsProcessor(DataProcessor):
    ...
    def get_labels(self):
        """设置当前数据集的标签"""
        import json
        # with open(os.path.join('cluster_api/dataset/clusters/20220808/fenlei1', 'labels_5.json'), 'r') as f_obj:
        #     labels = json.load(f_obj)
        df = pd.read_csv('/hy-tmp/julei/bert_torch_clf/data/train.csv')
        all_labels = df.label.to_list()
        labels = list(set(all_labels))
        return labels #["体育", "财经", "房产", "家居", "教育"]
