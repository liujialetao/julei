...
from cluster_api.metrics import acc_and_f1
from cluster_api.utils import output_modes as output_modes
from cluster_api.utils import processors as processors
from cluster_api.utils import convert_examples_to_features
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    if args.output_mode == "classification":
        e_x = np.exp(preds)
        probs1 = e_x / np.sum(e_x, axis=-1, keepdims=True)
        probs = np.max(probs1, axis=1)
        preds_label = np.argmax(preds, axis=1)
        pd.DataFrame({'labels': preds_label, 'scroe': probs}).to_csv(os.path.join(args.output_dir, "eval.csv"))
...
def predict(args, model, tokenizer, prefix=""):
    ...
    # pd.DataFrame(preds_label).to_csv(os.path.join(args.output_dir, "predicted.csv"), index=False)
    pd.DataFrame({'labels':preds_label, 'scroe':probs}).to_csv(os.path.join(args.output_dir, "predict.csv"))
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, predict=False):
    ...
    #     str(task)))
    cached_features_file ='{}_{}_{}'.format(
        list(filter(None, args.model_name_or_path.split('/'))).pop(),
        str(args.max_seq_length),
        str(task))
...
def start_train(train_args):
    ...
    args.gru_dropout = 0.1
    args.gru_hidden_size = 512
    args.gru_layers = 1
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        # torch.save(args, os.path.join(args.oufrom_pretrainedtput_dir, 'training_args.bin'))
